...
市場聚合在 live 與 fallback 使用同一套等權規則。只有狀態為「可執行」的主要計畫才計入多／空，其餘計入 neutral；多方占比減空方占比達 10 個百分點為「偏多」，低於 -10 個百分點為「偏空」，其餘為「震盪」。個別高風險占比達 30% 時整體為「高」；高風險低於 10% 且高加中風險低於 30% 時為「低」；其餘為「中」。live 個別風險依 1h 波動率分級（低於 2%／2–4%／4% 以上）；缺少歷史的 fallback 則依 24h 絕對漲跌（10%／18%）與成交量（1,000 萬／100 萬美元）分級。payload 的 `market.metrics` 公布總數、多／空／neutral、高／中／低風險的 counts 與 percentages。
