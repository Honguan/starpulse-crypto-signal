...
function candles(hourly) {
  return hourly
    .filter(([timestamp]) => timestamp % (4 * 60 * 60 * 1000) === 0)
...
const missingHour = uptrend.toSpliced(-2, 1);
assert.equal(strategyFor(missingHour, candles(uptrend), 148.1, uptrend.at(-1)[0]).planState, "資料不連續");

const missing4h = candles(uptrend).toSpliced(-2, 1);
assert.equal(strategyFor(uptrend, missing4h, 148.1, uptrend.at(-1)[0]).planState, "資料不連續");
assert.deepEqual(longResult.indicators.intervals, { indicators: 3_600_000, trend: 14_400_000 });
assert.equal(longResult.indicators.asOf1h, uptrend.at(-1)[0]);
assert.equal(longResult.indicators.asOf4h, candles(uptrend).at(-1)[0]);
